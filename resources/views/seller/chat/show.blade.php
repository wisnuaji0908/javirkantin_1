...
@section('title', 'Chat dengan ' . $buyer->name)
...
<style>
    ...
    .chat-header .buyer-info {
        margin-left: 10px;
    }
    .chat-header .buyer-info h5 {
        margin: 0;
        font-size: 1rem;
    }
    .chat-header .buyer-info p {
        margin: 0;
        font-size: 0.8rem;
        color: #666;
    }
...
@php
    // Misalnya buyer punya profile_image
        ...
        @if ($chats->isNotEmpty())
            @foreach ($chats as $chat)
                @php
                    ...
                    $senderName  = $isMe ? 'You' : $buyer->name;
                @endphp
...
<script>
    ...
    const authId   = {{ Auth::id() }}; // Seller ID
    ...
    socket.on('chat:NewMessage', (data) => {
        console.log("TERIMA EVENT (Seller):", data);
    ...
    function addMessageToUI(chat) {
        ...
        senderDiv.textContent = isMe ? 'You' : "{{ $buyer->name }}";
    ...
    chatForm.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch("{{ route('seller.chat.store', $buyer->id) }}", {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                ...
                // Opsional: tandai local echo error
            }
        } catch(err) {
            console.error(err);
        }
